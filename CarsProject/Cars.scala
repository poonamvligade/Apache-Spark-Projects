...
.map(x=>(x(0).toString,x(1).toString,x(2).toInt,x(3).toInt,x(4).toInt,x(5).toInt,x(6).toInt,x(7).toDouble,x(8).toInt,x(9).toString))
carsData.take(2)
...
carsData.map(x=>(x._10,1)).reduceByKey((x,y)=>x+y).collect

#filter out american cars
val americanCars=carsData.filter(x=>(x._10=="American"))

#count total american cars
americanCars.count()

#take sum of weights according to make
val makeWeightSum=americanCars.map(x=>(x._1,x._7)).combineByKey((x:Int) => (x, 1),
